...
import com.mz.schudlerserver.netty.connhandler.NettyServerHttpsHandler;
...
public class ConnHttpsNettyServer implements Runnable {

	private static final Logger logger = LoggerFactory.getLogger(ConnHttpsNettyServer.class);
	 ...
	 * @throws IOException 
	 */
	private void openHttpsNettyServer() throws IOException {
		...
		int nettyServerPort = Integer.parseInt(NettyConfig.getPropValue("NETTY_SERVER_HTTPS_PORT", "443"));
		...
		try {
			...
			serverBootstrap.group(bossGroup, workerGroup).channel(EpollServerSocketChannel.class)
					.handler(new LoggingHandler(LogLevel.INFO)).childHandler(new NettyServerHttpsHandler());
